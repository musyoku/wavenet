...
def train_audio(
	...
	quantized_signal = np.arange(1, padded_input_width * 16)
	...
	for pos in xrange(quantized_signal.size // (padded_input_width * batch_size)):
		for shift in xrange(padded_input_width):
			if (pos + 1) * padded_input_width * batch_size + shift + 1 < quantized_signal.size:
				...
				padded_quantized_input_signal_batch = data.onehot_pixel(padded_quantized_input_signal_batch, channels=params.audio_channels)
